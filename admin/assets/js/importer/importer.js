( function( $ ) {
  function _parseJSON( data ) {
    var m = data.match(
        /<!-- THIM_IMPORT_START -->(.*)<!-- THIM_IMPORT_END -->/ );
    try {
      if (m) {
        data = $.parseJSON( m[1] );
      } else {
  ...
  var Thim_Form_Import = Backbone.View.extend( {
    ...
    _stepByStep: function( step ) {
      ...
      this.current_request = $.ajax( {
        ...
        data: {
          action: 'thim_importer', time: ( new Date() ).getTime(),
        },
  ...
  var Thim_Importer_V2 = Backbone.View.extend( {
    ...
    initialize: function() {
      ...
      $( window ).
          on( 'thim_importer_complete', this.onImportSuccess.bind( this ) );
      $( window ).
          on( 'thim_uninstall_demo_successful',
