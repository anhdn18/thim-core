	...
	function thim_post_formats( $size ) {
		...
		switch ( get_post_format() ) {
			case 'image':
				...
				$html = sprintf( '<a class="post-image" href="%1$s" title="%2$s"><img src="%3$s" alt="%2$s"></a>',
			...
			case 'gallery':
				...
				foreach ( $images as $key => $image ) {
					if ( ! empty( $image['url'] ) ) {
						$html .= sprintf( '<li data-thumb="%s"><a href="%s" class="hover-gradient"><img src="%s" alt="gallery"></a></li>',
			...
			case 'audio':
				...
				if ( filter_var( $audio, FILTER_VALIDATE_URL ) ) {
					...
					wp_enqueue_script( 'thim-jplayer', THIM_CORE_ADMIN_URI . '/assets/js/jplayer/jquery.jplayer.min.js',
						array( 'jquery' ), '', true );
	...
	function thim_get_jplayer_interface() { ?>
        <div class="jp-gui jp-interface">
            <ul class="jp-controls">
                <li>
                    <a href="javascript:;" class="jp-previous" tabindex="1"><?php esc_attr_e( 'previous',
							'thim' ); ?></a>
                </li>
                ...
                <li>
                    <a href="javascript:;" class="jp-volume-max" tabindex="1"
                       title="<?php esc_attr_e( 'max volume', 'thim' ); ?>"><?php esc_attr_e( 'max volume',
							'thim' ); ?></a>
                </li>
            </ul>
            ...
            <ul class="jp-toggles">
                ...
                <li>
                    <a href="javascript:;" class="jp-shuffle-off" tabindex="1"
                       title="<?php esc_attr_e( 'shuffle off', 'thim' ); ?>"><?php esc_attr_e( 'shuffle off',
							'thim' ); ?></a>
                </li>
                <li>
                ...
                <li>
                    <a href="javascript:;" class="jp-repeat-off" tabindex="1"
                       title="<?php esc_attr_e( 'repeat off', 'thim' ); ?>"><?php esc_attr_e( 'repeat off',
							'thim' ); ?></a>
                </li>
            </ul>
        </div>
        ...
        <div id="<?php echo esc_attr( $id ); ?>" class="jp-audio">
            <div class="jp-type-playlist">
                ...
                <div class="jp-no-solution">
					<?php printf( __( '<span>Update Required</span> To play the media you will need to either update your browser to a recent version or update your <a href="%s" target="_blank">Flash plugin</a>.',
						'thim' ), 'http://get.adobe.com/flashplayer/' ); ?>
                </div>
            </div>
        </div>
		<?php
	...
	function thim_get_image( $args = array() ) {
		...
		if ( empty( $image_cache[ $key ] ) ) {
			...
			if ( ! isset( $html, $src ) && ( $args['scan'] ) ) {
				preg_match( '|<img.*?src=[\'"](.*?)[\'"].*?>|i', get_post_field( 'post_content', $args['post_id'] ),
					$matches );
