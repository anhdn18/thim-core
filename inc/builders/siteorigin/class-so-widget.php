...
if ( ! class_exists( 'Thim_Builder_SO_Widget' ) ) {
	...
	abstract class Thim_Builder_SO_Widget extends WP_Widget {
		...
		public function __construct() {
			...
			$options            = apply_filters('thim_so_custom_option_'.$this->base, $config_class::$options);
			   
			$this->form_options = Thim_Builder_SO_Mapping::mapping_options( $options );//if($this->config_class == 'Thim_Builder_Config_Countdown'){echo '<pre>';print_r($this->form_options);die;}
		...
		public function widget( $args, $instance ) {
			...
			echo '<div class="thim-widget-' . $this->base . ' template-' . $base_file. '">';
  			...
  			thim_builder_get_template( $base_file, array(
				$_params       => $params,
...
//						$instance[ $id ][ $i ] = $this->add_defaults( $instance[ $id ][ $i ], $field['fields'] );
//					}
		...
		function render_field( $name, $field, $value, $repeater = array(), $is_template = false ) {
			...
			switch ( $field['type'] ) {
				...
				default:
					...
					esc_html_e( 'Unknown Field', 'thim-core' );
					break;
			}
